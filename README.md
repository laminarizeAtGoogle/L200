...
|---|---|
| [`scripts/`](scripts/) | Operational scripts, including Argolis zero-privilege IAM boundary provisioner (`provision-argolis-env.sh`) and GitHub WIF setup (`setup-argolis-github-wif.sh`). |
| [`.github/workflows/terraform-apply.yml`](.github/workflows/terraform-apply.yml) | GitHub Actions workflow executing `terraform apply` upon merge to `main`. |
| [`.github/workflows/terraform-plan.yml`](.github/workflows/terraform-plan.yml) | GitHub Actions workflow executing `terraform plan` upon pull requests targeting `main`. |
...
| [`terraform/backend.tf.example`](terraform/backend.tf.example) | Example GCS remote backend configuration for state persistence. |
...
### 7. CI/CD: Automated Terraform Apply on Merge to Main

This workspace includes automated GitHub Actions workflows to validate and apply Terraform changes to your Argolis GCP project:
- **`terraform-plan.yml`**: Runs `terraform fmt`, `init`, `validate`, and `plan` on Pull Requests targeting `main`.
- **`terraform-apply.yml`**: Runs `terraform apply -auto-approve` immediately when a pull request is merged to `main`.

#### Security Pattern (Workload Identity Federation)
GitHub Actions authenticates to Argolis **without static service account keys** via Workload Identity Federation (OIDC). Authentication is strictly limited to your designated GitHub repository.

#### Step 1: Provision Argolis Environment
Run the provisioning script from Google Cloud Shell or an authenticated admin session with project admin permissions:

```bash
# Provision Workload Identity Federation, Service Account, and GCS state bucket:
...
In your target repository's **Settings -> Secrets and variables -> Actions**, add:

| Name | Description | Example |
|---|---|---|
| `GCP_PROJECT_ID` | Target Argolis GCP project ID | `my-argolis-sandbox-1234` |
| `GCP_WORKLOAD_IDENTITY_PROVIDER` | Full resource path of the OIDC provider | `projects/123456789/locations/global/workloadIdentityPools/github-actions-pool/providers/github-provider` |
| `GCP_SERVICE_ACCOUNT` | Deployment Service Account email | `github-terraform-deployer@my-argolis-sandbox-1234.iam.gserviceaccount.com` |
| `TF_STATE_BUCKET` | GCS bucket for remote Terraform state | `my-argolis-sandbox-1234-tfstate` |

*(Optional variables: `GCP_REGION` default `us-central1`, `GCP_ZONE` default `us-central1-a`)*

#### Step 3: Merge to Main
1. Open a pull request with your Terraform changes under `terraform/`.
2. Inspect the automated plan output generated by the `Terraform Plan` check.
3. Merge the pull request to `main`.
4. The `Terraform Apply on Main Merge` workflow will automatically trigger, acquire an ephemeral OIDC token via WIF, initialize against the remote GCS state bucket, and apply the infrastructure changes.

---

### 8. Google Agent Development Kit (ADK) & Python
...
### 9. Spec-Driven Development with OpenSpec
