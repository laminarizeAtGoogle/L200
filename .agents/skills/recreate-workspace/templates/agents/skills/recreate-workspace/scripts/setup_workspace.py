...
def setup_workspace(
    ...
    # 3.5 Install OpenSpec CLI via Airlock NPM Mirror
    ...
    code, stdout, stderr = run_command(
        [uv_bin, "pip", "install", "--env-file", ".env", "-r", "pyproject.toml"],
        cwd=target_dir,
        env=exec_env,
    )
    if code != 0:
        ...
        code, stdout, stderr = run_command(
            [uv_bin, "pip", "install", "google-adk[gcp]>=2.9.1", "pandas>=2.2.0", "ipykernel>=6.29.0"],
            cwd=target_dir,
            env=exec_env,
        )
